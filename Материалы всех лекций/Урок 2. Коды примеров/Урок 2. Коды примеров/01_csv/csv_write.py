...
# простая запись данных в файл .csv и вывод результата
# with open('kp_data_write_1_2.csv', 'w',encoding="utf-8",newline="") as f_n:
#     F_N_WRITER = csv.writer(f_n)
#     for row in DATA:
#         F_N_WRITER.writerow(row)


with open('kp_data_write_1.csv') as f_nb:
    print(f_nb.read())

# with open('kp_data_write_1_2.csv') as f_n:
#     print(type(f_n.read()))

#---------------------------------------------------------------------#
# QUOTE_NONNUMERIC - параметр явно указывает что представление будет строкой
